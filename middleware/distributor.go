	...
	return func(c *gin.Context) {
		...
		if ok {
			...
			if !ChannelStrategyEnabled {
				...
				filterChannels, err := circuitbreaker.GetManager().GetAvailableChannels(availableChannels)
				if err != nil {
					logger.SysError(fmt.Sprintf("Failed to filter channels through circuit breaker: %v", err))
				} else if len(filterChannels) == 0 {
					abortWithMessage(c, http.StatusServiceUnavailable, "所有渠道暂时不可用，请稍后重试")
					return
				}

				currentTime := time.Now().UnixMilli()
				channel, err = strategy.GetStrategySelector().SelectChannel(filterChannels, c)
				if err != nil {
		...
		// 获取响应头中的配额信息
		quota := model.GetQuotaHeader(c.Writer.Header(), requestModel, channel)

		// 记录时间窗口内最大消耗令牌数
		quota.RecordConsumedTokensInWindow(channel, requestModel)
		...
		logger.Debugf(ctx, "model: %s, using channel #%d, Level: %d, quota OTPM/TPM: %v, quota RPM: %v, consumed tokens: %v, resetTime: %s",
			requestModel, channel.Id, channel.KeyLevel,
			quota.RemainingTokens(channel, requestModel),
			quota.RemainingRequests(),
			quota.GetConsumedTokensInWindow(channel, requestModel),
			resetTimeStr)
		// 更新账户等级
		go func() {
